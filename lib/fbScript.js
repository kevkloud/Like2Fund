...
function donate(pid){
  chrome.runtime.sendMessage({meth : "donate", key: apikey, acc : dummyAccountID, fbID: pid}, function(response) {
...
function donate(pid){
  ...
  $.get('https://burning-torch-5051.firebaseio.com/monetizedPosts.json', function(success, error) {
    ...
    if (success) {
      ...
      var receiverID = success[pid].receiverID;
      console.log(receiverID);

      chrome.runtime.sendMessage({key: apikey, sender : dummySenderID, receiver : dummyReceiverID, fbID: pid}, function(response) {
      console.log(response.stat);
  });
    } else {
      console.log("Firebase get request failed: " + error);
    }
>>>>>>> c5848e0b542c2d859a7c7b96815c107b1f037f33
  });
